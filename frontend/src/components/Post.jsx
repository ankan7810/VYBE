...
function Post({ post }) {
  ...
  const handleComment=async ()=>{
    try {
      ...
      const updatedPosts=postData.map(p=>p._id==post._id?updatedPost:p)
      dispatch(setPostData(updatedPosts))
      setMessage("") // clear input after comment
    } catch (error) {
      console.log(error)
    }
  }
    ...
    <div className='w-[90%] flex flex-col gap-[10px] bg-white items-center shadow-2xl shadow-[#00000058] rounded-2xl pb-[20px]'>
      ...
      <div className='w-full h-[60px] flex justify-between items-center px-[20px] mt-[10px]'>
        <div className='flex justify-center items-center gap-[10px]'>
          ...
          {/* Hide comment icon when comment box is visible */}
          {!showComment && (
            <div className='flex justify-center items-center gap-[5px]' onClick={()=>setShowComment(true)}>
